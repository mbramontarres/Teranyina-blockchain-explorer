...
export default {
  ...
  data () {
    return {
        ...
        accounts: [],
        columns: ['Account', 'Available Balance', 'Free Balance','Locked Balance','reserved Balance','Total Balance','Nonce'],
        skip:0,
        page: 0,
    ...
    update (previous) {
      ...
      this.$apollo.queries.accounts.fetchMore({
        ...
        updateQuery: (previousResult, { fetchMoreResult }) => {
          ...
          if(fetchMoreResult.accounts.length==0){
      ...
      <div class="bg-white rounded-lg shadow-md">
        ...
        <div>
          <table id="tableData" :rows="rows" class=" table-fixed w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
            <tr v-for="account in accounts" :key="account.id" class="bg-white border-b items-center dark:bg-gray-800 dark:border-gray-700 truncate overflow:elipsis">
                <td class="text-center  items-center truncate pl-3">
                  <router-link class="flex truncate" :to='"/account/" + account.accountId' >
                  <jazzicon :address='"0x"+account.accountId' :seed="1" :diameter="15" /><div class="pl-1 text-blue-500 hover:underline truncate">{{account.accountId}}</div></router-link>
                    
                </td>
                <td class="px-3 text-center py-3 truncate">
                    {{formatAmount(account.availableBalance)}}
                </td>
                <td class="truncate text-center px-3">
                    {{formatAmount(account.freeBalance)}}
                </td>
                ...
                <td class=" text-center px-3 py-3 truncate">
